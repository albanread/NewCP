...
***)*
    ...
    BEGIN
        ...
        IF multiView OR (v # NIL) THEN
            ...
            IF q # NIL THEN
                ...
                IF Controls.link IN p.valid THEN inspect.link<u> := </u>p.link<u>$</u> END;

                IF Controls.label IN p.valid THEN inspect.label<u> := </u>p.label<u>$</u> END;

                IF Controls.guard IN p.valid THEN inspect.guard<u> := </u>p.guard<u>$</u> END;

                IF Controls.notifier IN p.valid THEN inspect.notifier<u> := </u>p.notifier<u>$</u> END;
    ...
    BEGIN
        ...
        IF multiView OR (v # NIL) THEN
            ...
            p.link<u> := </u>inspect.link<u>$</u>; p.label<u> := </u>inspect.label<u>$</u>;

            p.guard<u> := </u>inspect.guard<u>$</u>; p.notifier<u> := </u>inspect.notifier<u>$</u>;
