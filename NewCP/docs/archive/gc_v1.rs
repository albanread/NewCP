            ...
            while offset < self.bump {
                ...
                if !was_free && is_marked {
                    ...
                    if is_dead {
                        ...
                        // Defensive: a retired JIT module's TypeDesc may
                        // have been deallocated while this block was still
                        // tagged with its address. Reading `td.finalizer`
                        // would then dereference freed memory. Cross-check
                        // `td.size` against the block's payload bytes
                        // first; a mismatch means the descriptor is stale
                        // and we skip the finalizer call.
                        let td = (type_bits) as *const TypeDesc;
                        if !td.is_null()
                            && type_desc_looks_alive(td, block_size)
                        {
...
/// Returns `true` if `td` looks like a live `TypeDesc` for a block
/// whose total `block_size` is the value the block-header carries.
///
/// "Looks alive" here is a structural cross-check, not a guarantee:
/// we read `td.size` (offset 0) and confirm it's a non-negative
/// integer that fits inside the block's payload region with no more
/// than `BLOCK_ALIGN` bytes of slack.  When a JIT image has been
/// retired and its TypeDesc global deallocated, the bytes still
/// sitting at `td` are some other allocation's contents — almost
/// certainly with a wildly out-of-range first 8 bytes, so the check
/// fires.
///
/// # Safety
/// `td` must be a non-null pointer.  Reading it is the very thing
/// this function is checking is safe; we do exactly one volatile
/// read of an `isize` at offset 0.  On platforms where unmapped
/// pages would crash on read, this can still trap — the caller is
/// expected to guard the call with `cluster.contains(td)` or
/// equivalent for tighter safety.  In practice CP TypeDescs live
/// in JIT-emitted globals that stay mapped even after retirement;
/// the failure mode we're guarding against is *contents* changing
/// (the page being reused by the global allocator after the JIT
/// module's `OwnedJitModule` was dropped), not the page being
/// unmapped.
unsafe fn type_desc_looks_alive(td: *const TypeDesc, block_size: usize) -> bool {
    let header_size = std::mem::size_of::<BlockHeader>();
    let payload_bytes = block_size.saturating_sub(header_size);
    let claimed_size = unsafe { std::ptr::read_volatile(&(*td).size) };
    if claimed_size <= 0 {
        return false;
    }
    // We only enforce the in-bounds property: td.size must fit in
    // the block's payload region so that downstream
    // `payload.add(offset)` accesses stay within the allocation.
    // A *much* smaller td.size on a much larger block is plausible
    // (free-list slot oversized for the request), so we don't
    // bound the lower side — only the upper.
    (claimed_size as usize) <= payload_bytes
}
...
unsafe fn mark_object(start: *const u8) {
    ...
    while let Some(payload) = work.pop() {
        ...
        unsafe {
            ...
            // Sanity-check the TypeDesc before dereferencing.
            // A retired JIT module's TypeDesc global may have been
            // freed while heap blocks tagged with its address are
            // still live; reading `td.size` / `td.ptroffs` would
            // then dereference deallocated memory.
            if !type_desc_looks_alive(td, (*hdr).block_size) {
                continue;
            }
            ...
            for offset in (*td).pointer_offsets() {
                if (offset as usize) + std::mem::size_of::<*const u8>() > payload_bytes {
                    // Out-of-bounds offset: stale td whose ptroffs
                    // bytes happened to look plausible up front.
                    break;
                }
...
unsafe fn collect_inner(gc: &mut GcState, sp: usize) {
    if std::env::var("NEWCP_GC_DISABLE").is_ok() {
        return;
    }
    ...
    if base_stack != 0 && sp < base_stack && std::env::var("NEWCP_GC_NO_STACK_SCAN").is_err() {
    ...
    if std::env::var("NEWCP_GC_NO_MODULE_SCAN").is_err() {
        unsafe {
    ...
    if std::env::var("NEWCP_GC_NO_SWEEP").is_err() {
        unsafe {
            for cluster in &mut gc.clusters {
                let s = cluster.sweep();
