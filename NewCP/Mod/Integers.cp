...
**)
	...
	PROCEDURE OneDigitMult (IN a, b: Buffer; VAR c: Buffer);
	...
	PROCEDURE SimpleMult (IN a, b: Buffer; VAR c: Buffer);
	...
	PROCEDURE AddBuf (IN a, b: Buffer; VAR c: Buffer);	(* c := a + b *)
	...
	PROCEDURE AddToBuf (VAR a: Buffer; IN bIn: Buffer; shift: Index);	(* a := a + b * B^shift *)
		VAR i, n: Index; carry: Digit; b: Buffer;
	BEGIN
		(* NewCP records pass by reference; in BlackBox `b: Buffer` was a
		   value parameter so the local mutations to b.beg/b.len did not
		   leak to the caller. Take an explicit per-call copy to preserve
		   that semantics. *)
		b := bIn;
	...
	PROCEDURE SubtractFromBuf (VAR a: Buffer; IN b, c: Buffer);	(* a := a - b - c *)
		VAR i: Index; carry: Digit;
	BEGIN
	...
	PROCEDURE KStep (IN a, b: Buffer; VAR c: Buffer; IN stackIn: Buffer);
		VAR n2, i: Index; a0, a1, b0, b1, c0, c1, h, stack: Buffer;
	BEGIN
		(* `stack` carries an advancing high-water mark across recursive
		   KStep calls. In BlackBox it was a value parameter so each call
		   started from its caller's stack pointer and the recursive
		   advances were private. Make a per-call copy here to recreate
		   that semantics under NewCP's by-reference record ABI. *)
		stack := stackIn;
		ASSERT(a.len <= b.len, 20);
		...
		IF (ch > "0") & (ch <= "9") THEN
			...
			REPEAT
				...
				WHILE b # 0 DO
					...
					INC(i)
				END;
				new[(j - i) DIV DecPerDig] := dig; b := SHORT(SHORT(B DIV 10))
			UNTIL i = j;
			x := new
		ELSE x := zero
		END
	...
	BEGIN
		...
		IF j # 0 THEN
			DEC(j); d := x[j]; b := SHORT(SHORT(B DIV 10));
